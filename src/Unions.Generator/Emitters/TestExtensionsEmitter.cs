...
internal static class TestExtensionsEmitter
{
    public static string Emit(UnionModel model)
    {
        return $$"""
            {{AutoGeneratedMessage.Create(model.Name, nameof(TestExtensionsEmitter))}}
            ...
            public static class {{model.Name}}TestExtensions
            {
                ...
                {
            {{model.Cases.Select(caseModel => ShouldBe(caseModel, model.ArgumentName)).JoinParagraphs().Indent(2)}}
                }
            }
            """;
    }
